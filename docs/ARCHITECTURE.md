...
│               ├── destinations.py
│               └── venues.py     # GET/PATCH /venues..., POST /venues/{id}/validate, POST /venues/{id}/submit-for-review
...
│       ├── features/
│       │   └── venues/
│       │       ├── api.ts                # fetchVenues(), fetchVenue(id), updateVenue(id, patch), toVenuePatch(), validateVenue(id), submitVenueForReview(id)
...
│       │       └── workspace/
│       │           ├── VenueWorkspace.tsx     # stateful: no-selection/loading/error + useDraft + useUpdateVenue + useValidateVenue + useSubmitForReview + sections
│       │           ├── WorkspaceToolbar.tsx    # Edit / Cancel / Save Draft / Validate / Submit for Review + unsaved-changes + save/submit-error indicators
