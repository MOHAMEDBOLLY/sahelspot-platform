...
│       │   └── models.py    # Destination, Venue, PublishRevision, ActivityLogEntry ORM models
...
│       │   └── engine.py        # publish() (snapshot + atomic pointer flip), republish() (pointer-only), get_current_revision() (the only public-read lookup)
│       ├── activity/           # cross-cutting observability — independent of validation/, workflow/, and publishing/
│       │   └── service.py       # log_activity() — the one place an activity_log row is ever created; PLACEHOLDER_ACTOR = "system"
│       └── api/
│           ├── router.py    # aggregates route modules
│           ├── schemas.py    # VenueOut, VenueUpdate, DestinationOut, DestinationUpdate, DestinationRef, PublishRevisionOut, PublishRevisionDetail, PublishedVenueOut, ActivityLogEntryOut
│           └── routes/
│               ├── system.py    # GET / and GET /health
│               ├── destinations.py  # GET/PATCH /destinations... (Sprint 21 adds GET /{id} and PATCH /{id}, same pattern as venues.py)
...
│               └── activity.py   # GET /activity — read-only, newest first
