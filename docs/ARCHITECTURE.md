...
│       ├── auth/                # authentication — isolated behind one reusable dependency (Sprint 22)
│       │   └── dependencies.py   # CurrentUser, get_current_user() — verifies a Supabase JWT against SUPABASE_JWT_SECRET; no user table, no session store
